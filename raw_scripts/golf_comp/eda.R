...
shots <- read_csv(file = "~/Documents/sample data/hackathon-data-horsey.csv", col_types = cols(
...
initial_model <- gam(shots_remaining ~ s(left_to_pin_before, by = lie_before),
                     data = mutate(shots, lie_before = as.factor(lie_before)),
                     family = "poisson")

data_grid <- tidyr::expand(shots, lie_before, left_to_pin_before) %>%
  mutate(lie_before = as.factor(lie_before))
data_grid$expected_shots_remaining <- as.numeric(unname(predict(initial_model, data_grid, type = "response")))


ggplot(aes(x = left_to_pin_before / 36, y = expected_shots_remaining, color = lie_before), data = data_grid) + 
  geom_point(alpha = .4) +
...
data_grid %>%
  left_join(lie_range, by = c("lie_before")) %>% 
  filter(left_to_pin_before < p_upper, left_to_pin_before > p_lower) %>%
  ggplot(aes(x = left_to_pin_before / 36, y = expected_shots_remaining, color = lie_before)) +
  geom_point(alpha = .4) +
  coord_cartesian(xlim = c(0, 500), ylim = c(0, 7)) +
  guides(colour = guide_legend(override.aes = list(alpha = 1)))
...
plot_df <- data_grid %>%
  left_join(lie_range, by = c("lie_before")) %>% 
  filter(left_to_pin_before < p_upper, left_to_pin_before > p_lower) %>%
  filter(lie_range != "Other")

ggplot() + 
  geom_point(aes(x = left_to_pin_before / 36, y = expected_shots_remaining), color = "grey", data = select(plot_df, -lie_before)) +
  geom_point(aes(x = left_to_pin_before / 36, y = expected_shots_remaining, color = lie_before), alpha = .4, data = plot_df) +
  facet_wrap(~lie_before) +
  coord_cartesian(xlim = c(0, 500), ylim = c(0, 7)) +
  scale_color_discrete(guide = F) +
  theme_bw()
